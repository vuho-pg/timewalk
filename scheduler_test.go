...
func TestSchedule_String(t *testing.T) {
	...
	str := fmt.Sprintf("at 2023rd year, at December, at 22nd day, at Tuesday, at 3rd hour, at 11th minute, at 0th second, start from %v, end at %v with 1h0m0s duration", now.In(loc).Format(time.RFC850), now.Add(time.Hour).Format(time.RFC850))
